# [甲辰一师一课](https://github.com/qlteacher/sd1s1k-api-client-demo)
...
|名称|位置|类型|必选|说明|
|---|---|---|---|---|
|username|form-data|string| 是 |登录名|
|password|form-data|string| 是 |密码|
|grant_type|form-data|string| 是 |默认为：password|
|client_id|form-data|string| 是 |客户端编号|
|client_secret|form-data|string| 是 |客户端Secret|
...
|名称|类型|必选|约束|中文名|说明|
|---|---|---|---|---|---|
|access_token|string|true|none|Aouth2登录token|none|
|expires_in|number|true|none|超时时间|none|
|refresh_token|string|true|none|更新token用token|none|
|token_type|string|true|none|token类型|一般是：Bearer|
...
| 名称 | 值 | 说明 |
|---|---|---|
| Authorization | Bearer {access_token} | 这里值是登录请求的{token_type} {acess_token} |
